...
/* Includes ------------------------------------------------------------------*/
#include "core_cm3.h"
//#include "cmsis\cm3\DeviceSupport\MDR32F9Qx\startup\arm\system_MDR32F9Qx.h"
...
typedef struct
{
  __IO uint32_t RXTX; //Данные порта
  __IO uint32_t OE; //Направление порта
  __IO uint32_t FUNC;//Режим работы порта
  __IO uint32_t ANALOG;//Аналоговый режим работы порта
  __IO uint32_t PULL;//Подтяжка порта
  __IO uint32_t PD;//Режим работы выходного драйвера
  __IO uint32_t PWR;//Режим мощности передатчика
  __IO uint32_t GFEN;//Режим работы входного фильтра
}MDR_PORT_TypeDef;
